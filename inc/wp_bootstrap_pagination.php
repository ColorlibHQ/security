...
function security_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'security' ),
        'next_string'     => esc_html__( '&raquo;', 'security' ),
        'before_output'   => '<div class="pagination flex-m flex-w p-r-50">',
        'after_output'    => '</div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'security_pagination_defaults', $defaults )
    );
