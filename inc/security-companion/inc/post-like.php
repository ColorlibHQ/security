...
function security_sl_enqueue_scripts() {
	...
	wp_localize_script( 'security-post-likes', 'simpleLikes', array(
		...
		'unlike'  => esc_html__( 'Unlike', 'security' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_security_process_simple_like', 'security_process_simple_like' );
add_action( 'wp_ajax_security_process_simple_like', 'security_process_simple_like' );
function security_process_simple_like() {
	// Security
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'security' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'security' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'security' );
		$icon = $icon_empty;
	}
...
// User Profile List
add_action( 'show_user_profile', 'security_show_user_likes' );
add_action( 'edit_user_profile', 'security_show_user_likes' );
function security_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'security' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'security' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
