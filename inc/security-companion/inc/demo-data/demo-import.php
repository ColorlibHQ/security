...
function security_import_files() {
	...
	$demoImg = '<img src="'. SECURITY_COMPANION_DIR_URL . 'inc/demo-data/screen-image.png" alt="'.esc_attr__( 'Demo Preview Imgae', 'security' ).'" />';
  ...
  return array(
    array(
      ...
      'import_customizer_file_url'   => SECURITY_COMPANION_DIR_URL . 'inc/demo-data/security-customizer.dat',
...
function security_after_import_setup() {
	...
	$front_page_id = security_get_page_by_title( 'Home' );
	$blog_page_id  = security_get_page_by_title( 'Blog' );
...
function security_import_plugin_page_setup( $default_settings ) {
	...
	$default_settings['page_title']  = esc_html__( 'One Click Demo Import' , 'security' );
	$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'security' );
...
function security_demo_import_custom_scripts(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'security-demo-import' ){
		// style
		wp_enqueue_style( 'security-demo-import', SECURITY_COMPANION_DIR_URL . 'inc/demo-data/css/demo-import.css', array(), '1.0', false );
	}
