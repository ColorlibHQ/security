 ...
 */
    ...
    function security_meta_scripts() {
        wp_enqueue_style( 'security-meta-style', SECURITY_COMPANION_DIR_URL . 'inc/security-meta/assets/css/security-meta.css' );
        wp_enqueue_script( 'security-meta-script', SECURITY_COMPANION_DIR_URL . 'inc/security-meta/assets/js/security-meta.js', array('jquery'), '1.0', true );
    }
    ...
    function security_add_custom_meta_box() {
        ...
        add_meta_box("pageheader-meta-box", esc_html__( "Builder Page Header Settings", 'security' ), "security_bpheader_meta_box_markup", "page", "side", "high", null);
    }
    ...
    ?>
        <div class="header-opt header-show-opt">
            <p class="post-attributes-label-wrapper">
                <label for="pageheader-dropdown" class="post-attributes-label"><?php esc_html_e( 'Select Page Header', 'security' ); ?></label>
            </p>
            <?php 
            ...
            <select name="bpheadershow" class="security-admin-selectbox" id="page_header_selectbox">
                <option value="show" <?php echo esc_attr( $val == 'show' ? 'selected' : '' ); ?>><?php esc_html_e( 'Show', 'security' ); ?></option>
                <option value="hide" <?php echo esc_attr( $val == 'hide' ? 'selected' : '' ); ?> ><?php esc_html_e( 'Hide', 'security' ); ?></option>
            </select>

        </div>
        <div class="header-opt header-img">
            <p class="post-attributes-label-wrapper">
                <label for="pageheader-dropdown" class="post-attributes-label"><?php esc_html_e( 'Set Page Header Background', 'security' ); ?></label>
            </p>
            <?php 
            ...
            <select name="bpheaderimg" class="security-admin-selectbox" id="page_header_bg_selectbox">
                <option value="customize" <?php echo esc_attr( $val == 'customize' ? 'selected' : '' ); ?>><?php esc_html_e( 'From Customize', 'security' ); ?></option>
                <option value="featured" <?php echo esc_attr( $val == 'featured' ? 'selected' : '' ); ?> ><?php esc_html_e( 'Featured Image', 'security' ); ?></option>
            </select>
